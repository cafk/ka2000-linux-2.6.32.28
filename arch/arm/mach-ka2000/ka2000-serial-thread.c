...
struct task_struct *tx_task = NULL;
 ...
 */
#define uart_word_write(a,v) 	__raw_writel(v, IO_ADDRESS(KA2000_UART0_BASE+a))
#define uart_word_read(a)  		__raw_readl(IO_ADDRESS(KA2000_UART0_BASE+a))
...
static void serial_out(struct uart_port *port, const char ch)
{
    ...
    {
		barrier();
        if ((UART_GET_LSR(port) & URLS_URTHRE))  // Read Line Status Register
            break;	
    }
...
static int transmit_chars(struct uart_port *port)
{
    ...
    count = 1024; // + xmit->tail - xmit->head;
    do
    {	
...
static int receive_chars(struct uart_port *port)
{
#if LINUX_VERSION_CODE > KERNEL_VERSION(2,6,28)
...
static int ka2000_uart_tx_task(void * data)
{    
	struct uart_port *port = (struct uart_port *)data;
	const unsigned long jtime = msecs_to_jiffies(100);
	unsigned long sleep_count = 10;
	...
	while (!kthread_should_stop()) {
        ...
        if (!kthread_should_stop())
	           schedule_timeout(sleep_count);

        set_current_state(TASK_RUNNING);

		//if (UART_GET_LSR(port) & URLS_URDR)
		//	receive_chars(port);
        if (transmit_chars(port) == 0)
        {
        	//UART_PUT_CHAR(port, '$');
			sleep_count = jtime;
        }	
...
/* boot time printk will call this function */
...
static int __init ka2000uart_init(void)
{
	...
	tx_task = kthread_create(ka2000_uart_tx_task, &ka2000uart_ports[0], "uart_tx");
	if (IS_ERR(tx_task)){
	     ret = PTR_ERR(tx_task);
		 tx_task = NULL;
		 return -1;
