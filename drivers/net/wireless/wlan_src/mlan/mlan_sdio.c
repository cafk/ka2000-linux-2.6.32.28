...
#undef ENTER
#undef LEAVE
#define ENTER(); trace_t("mlan sdio", __LINE__); //printk("%s\n", __FUNCTION__);
#define LEAVE(); trace_t("mlan sdio", __LINE__);
#endif

extern unsigned long enter_time;
#define mlan_word_write(a,v)    __raw_writel(v, IO_ADDRESS(a))
#define mlan_word_read(a)       __raw_readl(IO_ADDRESS(a))
...
#define ENTER() \
             enter_time = mlan_word_read(0xa000204c); \
             printk("%s E %ld\n",__FUNCTION__, enter_time);

#define LEAVE() \
             printk("%s L %ld\n",__FUNCTION__, enter_time+mlan_word_read(0xa000204c));
#endif
