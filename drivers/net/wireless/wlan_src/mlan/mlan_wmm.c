...
#undef ENTER
#undef LEAVE
#define ENTER(); trace_t("wmm enter", __LINE__); //printk("%s\n", __FUNCTION__);
#define LEAVE(); trace_t("wmm leave", __LINE__);
/********************************************************
    Local Variables
********************************************************/
...
wlan_send_single_packet(pmlan_private priv, raListTbl * ptr, int ptrindex)
{
	...
	//printk("wlan_send_single_packet %d\n", tx_param.next_pkt_len);
...
wlan_dequeue_tx_packet(pmlan_adapter pmadapter)
{
    ...
    if (!ptr->is_11n_enabled || wlan_is_bastream_setup(priv, ptr, tid)
        ...
        || ((priv->sec_info.ewpa_enabled == MFALSE) &&
           ...
           // printk("wlan_dequeue_tx_packet 1\n");
...
#define MIN_NUM_AMSDU 2
        if (wlan_is_amsdu_allowed(priv, ptr, tid) &&
           ...
           // printk("wlan_dequeue_tx_packet 2\n");
            wlan_send_single_packet(priv, ptr, ptrindex);
        }
    }
...
//printk("LEAVE wlan_wmm_process_tx\n");
    LEAVE();
    return;
}
